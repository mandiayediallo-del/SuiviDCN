/* DCN V16.3 — synchronisation différentielle App -> Sheets. */
(function(){
  'use strict';
  ...
  function resetBaseline(db){baseline=clone(db);pending=false;lastError=null;if(window.DCN_STATE)window.DCN_STATE.persistCache(db);if(typeof markSaved==='function')markSaved();}
  ...
  async function flush(db){
    ...
    if(!operations.length){
      pending=false;status('● Synchronisé','ok');
      if(window.DCN_STATE)window.DCN_STATE.persistCache(db);
      if(typeof markSaved==='function')markSaved();
      return {ok:true,applied:0,changedTables:[]};
    }

    const check=window.DCN_PERMISSIONS?.validateOperations?.(operations);
    if(check && check.ok===false){
      pending=false;lastError=new Error(check.message||'Modification non autorisée');
      status('● Modification refusée','error');
      if(typeof toast==='function')toast(lastError.message,'err');
      throw lastError;
    }
    ...
    try{
      ...
      baseline=clone(db);lastError=null;
      if(window.DCN_STATE)window.DCN_STATE.persistCache(db);
      if(typeof markSaved==='function')markSaved();
      window.DCN_BISYNC?.markPushed?.(res,operations);
      status('● Synchronisé','ok');
      return res;
    }catch(e){
      lastError=e;pending=true;status('● Non synchronisé','error');console.error('DCN sync',e);
      if(typeof toast==='function')toast('Synchronisation impossible : '+e.message,'err');
      throw e;
    }finally{
  ...
  function adoptField(db,field){
    if(!db||!field)return;
    if(!baseline)baseline=clone(db);else baseline[field]=clone(db[field]);
    if(window.DCN_STATE)window.DCN_STATE.persistCache(db);
  }
  function getState(){return {pending,inFlight,lastError:lastError?lastError.message:null};}
  window.DCN_SYNC={init,resetBaseline,schedule,flush,getState,adoptField};
})();
